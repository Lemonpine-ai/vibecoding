const OPENAI_API_KEY_FOR_FLOATING_CHATBOT = "YOUR_OPENAI_API_KEY_HERE";
...
async function sendMessageToOpenAIFromFloatingChatbot(userMessageText) {
  ...
  try {
    const apiResponse = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      headers: {
        ...
        "Authorization": `Bearer ${OPENAI_API_KEY_FOR_FLOATING_CHATBOT}`
      },
      body: JSON.stringify({
        model: "gpt-3.5-turbo",
...
async function handleFloatingChatbotSendMessage() {
  ...
  try {
    const assistantReply = await sendMessageToOpenAIFromFloatingChatbot(userInputText);
